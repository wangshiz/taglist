...
function changeTag(){
	...
	var labelList = tagValue.split(/[,;，；]/);
      ...
      if(i==0){
        var text = labelList[i].replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
        headText += text;
      }else{
        var text = labelList[i].replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g, '');
        headText += (','+text);
...
$(function(){
	var arr = getTagsArr();
	for(var i in arr){
        $("#addTagButton").before("<div class='tagClass'><span class='ontag select-tag' contenteditable='true' onkeyup='noComma(this)' onClick=clickIt(this) onBlur=blurIt(this)>"+arr[i]+"</span><span class='deleteTag' onmousedown='deleteOne(this)'>×</span></div>");
      }
      if(arr.length>=5){
        $("#addTagButton").hide();
      }
...
function findAllTag(obj){
    //后台返取得arr
    var arr = [123,345,678,101112,"我是陈小春","大师兄","渣渣辉","dadsadasosaosadk", "我是掉dsadasdadsad"];
    //表面上获得arr
    var tagArr = obj;
    if(tagArr == null){
      tagArr = [''];
    ...
    for(var i in arr){
      ...
      if(isRepeat){
        $("#allTag").append('<div class="undertag select-tag" contenteditable="false">'+tagName+'</div>');
      }else{
        $("#allTag").append('<div class="undertag no-select-tag" contenteditable="false">'+tagName+'</div>');
      }
    }  
  verifyTagCount();
}
...
//控制上面的标签编辑
function addTag(){
  $("#addTagButton").before("<div class='tagClass'><span class='ontag select-tag' contenteditable='true' onkeyup='noComma(this)' onClick=clickIt(this) onBlur=blurIt(this)></span><span class='deleteTag' onmousedown='deleteOne(this)'>×</span></div>");
...
function addOriginalTag(obj){
  ...
  if(verify){
    ...
    $("#addTagButton").before("<div class='tagClass'><span class='ontag select-tag' contenteditable='true' onkeyup='noComma(this)' onClick=clickIt(this) onBlur=blurIt(this)>"+tagName+"</span><span class='deleteTag' onmousedown='deleteOne(this)'>×</span></div>");
